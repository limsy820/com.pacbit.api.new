...
public class SeedXUtil {

    // Round keys for encryption or decryption
    static int pdwRoundKey[] = new int[32];
    // User secret key
    static byte pbUserKey[]  = "1234567890123456".getBytes();
    // input plaintext to be encrypted
    //byte pbData[]     = "1234567890123456".getBytes();

    byte pbCipher[]   = new byte[16];
    byte pbPlain[]    = new byte[16];

    private static SeedXUtil seedXUtil = null;

    public static SeedXUtil getinstance(){
        synchronized(SeedXUtil.class){
            if(seedXUtil == null){
                seedXUtil = new SeedXUtil();
...
// reorder for little endian
    ...
    public String SeedEncrypt(String str) throws Exception {
        ...
        for(int i = 0; i < array.length; i += 16) {
            ...
            if(i + 16 < array.length) {//16바이트 이상일 경우 그대로 암호화
                byte[] arr = new byte[16];
                System.arraycopy(array, i, arr, 0, 16);
                SeedEncrypt(arr, pdwRoundKey, pbCipher);
                hexStr += byteArrayToHex(pbCipher).toUpperCase();

            } else {//16바이트 이하 일 경우 공백을 채워서 암호화
                byte[] arr = new byte[16];
        ...
        //System.out.println("암호화 : " + str + " > " + hexStr);

        return hexStr;

    }

    public String SeedDecrypt(String str) throws Exception {

        byte[] array = hexToByteArray(str);
        String decStr = "";
        ...
        for(int i = 0; i < array.length; i += 16) {
            if(i + 16 < array.length) {
                byte[] arr = new byte[16];
                System.arraycopy(array, i, arr, 0, 16);
                SeedDecrypt(arr, pdwRoundKey, pbCipher);
                decStr += new String(pbCipher);
            } else {
                byte[] arr = new byte[16];

                System.arraycopy(array, i, arr, 0, 16);
                SeedDecrypt(arr, pdwRoundKey, pbCipher);
                String s = new String(pbCipher);
                byte[] arr2 = null;
                for(int j = 0; j < pbCipher.length; j++) {
                    if(pbCipher[j] == 0) {
                        arr2 = new byte[j];
                        System.arraycopy(pbCipher, 0, arr2, 0, j);
                        break;
                    }
                }
                if(arr2 != null) {
                    decStr += new String(arr2);
                } else {
                    decStr += s;
                }
        ...
        //System.out.println("복호화 : " + str + " > " + decStr);
